  ...
  return (
    <section className="w-full max-w-[1500px]">
      <Swiper
        ...
        className="mySwiper md:h-[500px] h-[300px] mb-8 md:mb-4"
      >

        {data && data.map(movie => {
          return (
            <SwiperSlide key={movie.id} className="w-full h-full bg-blue-500 relative">
              <div className="w-full h-full">
                <img src={movie.cover} alt="slider" className="w-full h-full object-cover" />
              </div>
              <div className="text-sm md:text-base select-none w-full h-full absolute top-0 right-0 bg-gradient-to-t from-[#fcfcfc]">
