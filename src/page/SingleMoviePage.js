      ...
      {data ? (
        <section className="w-full max-w-[1600px] mx-auto">
                  ...
                  <div className="w-full flex md:flex-col">
                    <div className="w-full md:border-b border-gray-600 md:border-gray-400 md:pb-4">
                      <div className="w-full flex items-center justify-center md:justify-start gap-x-4 py-2">
                        <div>
                          {data.imdb}
                          <span className="bg-yellow-500 p-2 rounded-md text-sm ml-2">
                            imdb
                          </span>
                        </div>
                        <div className="flex items-center p-2">
                          <FiHeart className="bg-green-400 rounded-md text-3xl p-1 ml-2" />
                          {data.satisfaction}
                        </div>
                      </div>
          ...
          <article className="md:container pb-8 w-full flex flex-col md:flex-row items-center px-4 border-b border-gray-500 mb-6">
            ...
            <div className="w-full">
              <span className="flex items-center gap-x-2 text-xl mt-6 md:mt-0">
                <FiYoutube className="text-xl" />
                داستان {data.type}
              </span>
              <p className="text-sm text-justify mt-4 text-gray-400 leading-6">
                {data.story}
              </p>
              <span className="flex items-center gap-x-2 text-xl mt-6">
                <FiYoutube className="text-xl" />
                درباره {data.type}
              </span>
              <p className="text-sm text-justify mt-4 text-gray-400 leading-6">
          ...
          <article className="px-4 md:container flex flex-col items-center gap-y-3 py-4">
            ...
            {data.coming === false ? (
              data.downloads.length ? (
                      ...
                      <div className="flex items-center md:justify-end justify-center flex-wrap  w-full gap-2">
                        {item.linkDownload.map((link, index) => {
                          return (
                            <button
                              key={index}
                              className="bg-green-500 text-white flex items-center gap-x-1 px-4 py-2 rounded-md"
                            >
                              {link.quality}
                              <FiDownload />
                            </button>
                ...
                <span className="flex items-center rounded-lg text-justify justify-start w-full p-4 text-sm bg-zinc-900 text-red-500">
                  لینک دانلود وجود ندارد{" "}
                </span>
              )
            ) : (
              <span className="flex items-center rounded-lg text-justify justify-start w-full p-4 text-sm bg-zinc-900 text-green-500">
                لینک های دانلود بعد از انتشار فیلم در دسترس قرار میگیرد
              </span>
            )}
          </article>
            ...
            <div className="md:container p-4">
              <div className="w-full">
                <div className="w-full flex items-center justify-between">
                  ...
                  <button
                    onClick={handleBoxComment}
                    className="flex items-center gap-x-2 text-green-500 border border-green-500 py-2 px-4 hover:bg-green-500 hover:text-white duration-300 rounded-md"
                  >
                    {isOpenBoxComment ? (
                      <>
                ...
                <div
                  ref={boxComment}
                  className="flex justify-start flex-wrap gap-3 max-h-0 overflow-hidden "
                >
                  <span className="w-full text-xl mb-4 mt-8">
                    دیدگاهتان را بنویسید
                  </span>
                  <textarea
                    className="p-3 bg-zinc-900 outline-none w-full"
                    placeholder="این فیلم چطور بود؟"
                  />
                  <input
                    type="text"
                    className="w-full md:max-w-[425px] p-3 bg-zinc-900 outline-none"
                    placeholder="نام شما"
                  />
                  <button className="rounded w-full md:w-auto text-sm py-3 px-8 bg-green-500 hover:bg-green-700 duration-300 outline-none">
                    ثبت دیدگاه
                  </button>
                </div>
                {/* List Comment */}
